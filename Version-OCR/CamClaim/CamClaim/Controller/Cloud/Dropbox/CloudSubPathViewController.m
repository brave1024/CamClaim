...
@interface CloudSubPathViewController () <DBRestClientDelegate, UITableViewDataSource, UITableViewDelegate, UIActionSheetDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@property (nonatomic, strong) NSMutableArray *arrayPaths;

@property (nonatomic, copy) NSString *cloudKey;

@end

@implementation CloudSubPathViewController
...
- (void)viewDidLoad {
    ...
    NSLog(@"dirPath:%@", self.dirPath);
    self.arrayPaths = [[NSMutableArray alloc] initWithObjects:self.dirPath, nil];
...
- (void)getCloudContent
{
    ...
    [self.restClient loadMetadata:[self.arrayPaths lastObject]];
}
...
- (void)uploadFile:(id)data
{
    ...
    if (data != nil)
    {
        ...
        if ([data isKindOfClass:[NSString class]] == YES)
        {
            ...
            NSString *destinationPath = [self.arrayPaths lastObject];
            [self.restClient uploadFile:fileName toPath:destinationPath withParentRev:nil fromPath:imgPath];
            
            [MRProgressOverlayView showOverlayAddedTo:self.viewContent title:kUploading mode:MRProgressOverlayViewModeIndeterminateSmall animated:YES];
            self.navView.btnMore.enabled = NO;
        }
        else
        {
