...
//    [self.btnLogin setBackgroundImage:imgBtn_ forState:UIControlStateHighlighted];
    ...
    [self.btnForget setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    
//    [self.btnForget setTitle:@"哈哈哈哈哈哈哈哈哈" forState:UIControlStateNormal];
//    [self.btnForget setBackgroundColor:[UIColor redColor]];
...
//    NSLog(@"<%@> <%@>", strTest, strTest_);
    ...
    strValue = locatizedString(@"forget password");
    [self.btnForget setTitle:strValue forState:UIControlStateNormal];
}
...
- (IBAction)loginAction
{
    ...
    if (status != 0)
    {
        switch (status) {
            ...
            case 5: {
                //[self toast:@"账号必须为邮箱"];
                NSString *strTip = locatizedString(@"account_must_email");
                [self toast:strTip];
                break;
            }
            case 6: {
        ...
        if (isSucceed == YES)
        {
            if (data != nil)
            {
                ...
                if (response.status == 1)
                {
                    ...
                    if ((self.userInfo.realname != nil && self.userInfo.realname.length > 0)
                        && (self.userInfo.email != nil && self.userInfo.email.length > 0)
                        && (self.userInfo.phone != nil && self.userInfo.phone.length > 0)
                        && (self.userInfo.department != nil && self.userInfo.department.length > 0))
                    {
                        // 个人资料完整
                        numberFlag = @(NO); // 不是新用户
                    }
                    ...
                    [self dismissViewControllerAnimated:YES completion:^{
                        ...
                        else
                        {
                            ...
                            NSString *strTip = locatizedString(@"login_success_set_pincode");
                            [self toast:strTip];
...
- (int)checkContent
{
    ...
    if ([NSString checkMailAddress:self.textfieldAccount.text] == NO)
    {
        return 5;
    }
