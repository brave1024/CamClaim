...
@interface FoodDetailViewController () <UITableViewDataSource, UITableViewDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate>
...
- (void)initData
{
    self.txtfieldHotel.text = nil;
    self.txtfieldCompany.text = nil;
    self.txtfieldCost.text = nil;
    self.txtfieldUserCompany.text = nil;
    
    // 显示
    self.lblStart.text = self.item.qaddress;
    self.txtfieldHotel.text = self.item.store;
    self.txtfieldCompany.text = self.item.clientcompany;
    self.txtfieldCost.text = self.item.canmoney;
    self.txtfieldUserCompany.text = self.item.usercompany;
    
    if ([self.item.eatway isEqualToString:@"早餐"] == YES)
    {
        self.flagFood = typeFoodBreakfast;
        //self.lblFoodType.text = @"早餐";
        NSString *breakfast = locatizedString(@"breakfast");
        self.lblFoodType.text = breakfast;
    }
    else if ([self.item.eatway isEqualToString:@"午餐"] == YES)
    {
        self.flagFood = typeFoodLunch;
        //self.lblFoodType.text = @"午餐";
        NSString *lunch = locatizedString(@"lunch");
        self.lblFoodType.text = lunch;
    }
    else if ([self.item.eatway isEqualToString:@"晚餐"] == YES)
    {
...
- (IBAction)selectFootType
{
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"请选择"
                                               ...
                                               destructiveButtonTitle:nil
                                                    otherButtonTitles:@"早餐", @"午餐", @"晚餐", nil];
