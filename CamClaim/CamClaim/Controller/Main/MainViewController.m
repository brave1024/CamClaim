...
@interface MainViewController () <NavViewDelegate, UITableViewDataSource, UITableViewDelegate, IIViewDeckControllerDelegate, SimpleCamDelegate, G8TesseractDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIActionSheetDelegate, ISSShareViewDelegate>
...
@property (nonatomic, strong) IBOutlet UILabel *lblRejected;
@property (nonatomic, strong) IBOutlet UILabel *lblApproved;
@property (nonatomic, strong) IBOutlet UILabel *lblApproving;
...
{
    NSString *strValue = locatizedString(@"ocr_capture");
    [self.btnCapture setTitle:strValue forState:UIControlStateNormal];
    
    strValue = locatizedString(@"claim_rejected");
    self.lblRejected.text = strValue;
    
    strValue = locatizedString(@"claim_approved");
    self.lblApproved.text = strValue;
    
    strValue = locatizedString(@"claim_approving");
    self.lblApproving.text = strValue;
    
    strValue = locatizedString(@"claim_pending");
    self.lblPending.text = strValue;
}
...
- (void)showShareView
{
//    [self shareToLinkedInClick];
//    return;
    
//    [self btnClickHanlder:nil];
//    return;
...
// LinkedIn
// 分享失败,错误码:0,错误描述:Unable to verify access token

- (void)btnClickHanlder:(id)sender
{
...
- (void)share:(id)sender
{
    ...
    id <ISSContent> content = [ShareSDK content:@"hello world"
                                 defaultContent:nil
                                          image:[ShareSDK imageWithPath:[[NSBundle mainBundle] pathForResource:@"明日香" ofType:@"jpg"]]
                                          title:@"hello"
...
{
    //修改分享编辑框的标题栏颜色
    viewController.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:(CGFloat)248/255 green:(CGFloat)127/255 blue:(CGFloat)31/255 alpha:1];
...
- (void)checkUserCurrentStats
{
    if (kAppDelegate.isLogin == YES)
    {
        ...
        [MRProgressOverlayView showOverlayAddedTo:self.view.window title:strLoading mode:MRProgressOverlayViewModeIndeterminateSmall animated:YES];
    }
    else
    {
        // 自动登录已完成...
        ...
        else
        {
            ...
            NSString *strTip = locatizedString(@"login_fail");
            [self toast:strTip];
...
- (void)showUserInfo
{
    ...
    if (user.userInfo.img != nil && user.userInfo.img.length > 0)
    {
        ...
        [self.imgviewAvatar sd_setImageWithURL:[NSURL URLWithString:user.userInfo.img] placeholderImage:[UIImage imageNamed:@"img_avatar"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (image != nil)
            {
        ...
        else
        {
            ...
            else
            {
                if (showloading == YES)
                {
                    ...
                    NSString *strTip = locatizedString(@"load_fail");
                    [self toast:strTip];
                }
            }
        }
