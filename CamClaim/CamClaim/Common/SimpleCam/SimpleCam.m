...
@interface SimpleCam () <NavViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
{
    // Measurements
    CGFloat screenWidth;
...
#pragma mark - Custom Capture Button

- (void)addCustomCaptureButton
{
    ...
    self.btnCapture.frame = CGRectMake((kScreenWidth-60)/2, kScreenHeight-100, 60, 60);
    [self.btnCapture setBackgroundImage:[UIImage imageNamed:@"btn_capture"] forState:UIControlStateNormal];
    [self.btnCapture addTarget:self action:@selector(capturePhotoAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.btnCapture];
    
    self.btnCancel = [UIButton buttonWithType:UIButtonTypeCustom];
    self.btnCancel.frame = CGRectMake(10, kScreenHeight-60, 80, 38);
    [self.btnCancel setImage:[UIImage imageNamed:@"btn_close"] forState:UIControlStateNormal];
    ...
    self.btnSave.frame = CGRectMake(kScreenWidth-90, kScreenHeight-60, 80, 38);
    [self.btnSave setImage:[UIImage imageNamed:@"btn_confirm"] forState:UIControlStateNormal];
    //[self.btnSave setTitle:@"Save" forState:UIControlStateNormal];
    //[self.btnSave setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
...
- (void)drawCustomButton
{
    [UIView animateWithDuration:self.controlAnimateDuration delay:0 options:UIViewAnimationOptionCurveEaseOut  animations:^{
        ...
        {
            // Hide
            for (UIButton *btn in @[_btnCapture])
            {
                btn.hidden = YES;
            }
            
            // Show
            for (UIButton *btn in @[_btnCancel, _btnSave])
            {
                btn.hidden = NO;
            }
        }
